...
public class JenaEdgeTest extends JenaGeneralGraphManipulatorTest{
    ...
    public void can_add_relation() {
        Vertex secondVertex = vertexA.addVertexAndRelation().destinationVertex();
        Vertex thirdVertex = secondVertex.addVertexAndRelation().destinationVertex();

        Integer numberOfEdgesAndVertices = graphManipulator.numberOfEdgesAndVertices();
        Edge newEdge = thirdVertex.addRelationToVertex(vertexA);
